...
import {
  ...
  useNavigate,
  useLocation,
} from "react-router-dom"; // No BrowserRouter here
...
import ReliefAllocation from "@/app/components/ReliefAllocation";
...
export default function App() {
  ...
  const [user, setUser] = useState<{
    ...
    return saved
      ? JSON.parse(saved)
      : { isLoggedIn: false, role: null, email: null };
  });
  ...
  const handleLogin = (role: string, email?: string) => {
    setUser({ isLoggedIn: true, role, email: email || null });
        ...
        <Routes>
          ...
          <Route
            ...
            element={
              ...
              ) : user.role === "teacherDash" ? (
                <Navigate to="/teacher-dash" replace />
              ) : (
                <Navigate to="/homeroom" replace />
              )
            }
          />
          ...
          <Route path="/grading" element={<StudentMarkEntry />} />
          <Route path="/accounts" element={<ManageAccounts />} />
          <Route path="/leave" element={<LeaveForm />} />
          <Route path="/relief" element={<ReliefAllocation />} />
